...
const FetchGistData = (id) => {
  return new Promise( 
    (resolve, reject) => {
      log("requesting load of gist", id);

      $.get({ url: `${gist_api_url}/${id}`,})
        .done(reply => {
          log(`Got reply for gist '${id}'`);
...
const FillTruncatedGists = (data, callback) => {
  ...
  for (var key in files) {
    ...
    {
      log(`file ${key} was truncated. Fetching full content`);
  ...
  if(to_fetch_count === 0)
  {
    log("No files were truncated, so nothing more to fetch");
    callback(data);
    return;
  }
  ...
  Promise.all(promises).then( 
    ...
    (vals) => {
      ...
      log(`All ${to_fetch_count} truncated files have been fetched`);
      callback(data);
    }, 
    // Error
    (reason) => {
